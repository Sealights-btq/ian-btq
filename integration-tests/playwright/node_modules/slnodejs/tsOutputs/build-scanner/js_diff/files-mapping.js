...
const { SourceMapConfigDto } = require('../../common/source-map-config-dto');
...
FilesMapping.prototype._scanWorkspaceForIncludedFilesAndPopulateFilesArray = function () {
    ...
    context.options.includedFiles.forEach(function (inclusionPattern) {
        var matchedFiles = globby.sync(inclusionPattern, {
            cwd: context.options.workspacepath,
            ignore: context.options.excludedFiles,
            dot: context.options.scanDotFolders,
            onlyFiles: false, // Added to keep the same behavior as the old glob package which includes directories
        });
...
FilesMapping.prototype._generateFileSignature = function (absolutePath) {
    ...
    var opts = {
        ...
        hashIncludeNestedMethods: this.options.hashIncludeNestedMethods,
        enableLineCoverage: this.options.enableLineCoverage,
        recognizeBabelCreateClass: this.options.recognizeBabelCreateClass,
    };
...
FilesMapping.prototype._createActualFile = function (filename, fullFilename) {
    return new ActualFile(filename, fullFilename, SourceMapConfigDto.from(this.handlerParams.cfg));
};
